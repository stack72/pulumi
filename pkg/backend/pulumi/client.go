...
type serviceUpdate struct {
	updateID         client.UpdateIdentifier
	requiredPolicies []engine.RequiredPolicy
	tokenSource      *tokenSource
	client           *client.Client
...
func (c *Client) StartUpdate(ctx context.Context, kind apitype.UpdateKind, stackID backend.StackIdentifier,
	...
	policies := make([]engine.RequiredPolicy, len(reqdPolicies))
	for i, policy := range reqdPolicies {
		policies[i] = newRequiredPolicy(c.client, policy, update.Owner)
	}
